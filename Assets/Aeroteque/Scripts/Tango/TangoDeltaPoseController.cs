...
public class TangoDeltaPoseController : MonoBehaviour, ITangoPose
{
    ...
    public Vector2 speedMultiplier = new Vector2(1f, 1f);
    /// <summary>
    ...
    private void _UpdateTransformationFromPose(TangoPoseData pose)
    {
        ...
        deltaPosition.Set(deltaPosition.x * speedMultiplier.x, deltaPosition.y * speedMultiplier.y,
                deltaPosition.z * speedMultiplier.x);

        if (m_characterMotion && m_characterController != null)
        {
            m_characterController.Move(deltaPosition);
            rotationTarget.rotation = deltaRotation * rotationTarget.rotation;
        }
        else
        {
